...
class SettingProvider {
	 ...
	 * @abstract
	 */
	destroy() { throw new Error(`${this.constructor.name} não tem um método de destruição.`); }
	 ...
	 * @abstract
	 */
	set(guild, key, val) { throw new Error(`${this.constructor.name} não possui um método definido.`); }
	 ...
	 * @abstract
	 */
	remove(guild, key) { throw new Error(`${this.constructor.name} não tem um método de remoção.`); }
	 ...
	 * @abstract
	 */
	clear(guild) { throw new Error(`${this.constructor.name} não tem um método claro.`); }
	...
	static getGuildID(guild) {
		...
		throw new TypeError('Aliança inválida especificada. Deve ser uma instância, ID da guilda, "global" ou nulo.');
	}
}
