...
Somente administradores podem usar este comando.
	...
	run(msg, args) {
		if(!args.cmdOrGrp.isEnabledIn(msg.guild, true)) {
			return msg.reply(
				`O \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'Comando' : 'group'} já está desativado.`
			);
		}
		if(args.cmdOrGrp.guarded) {
			return msg.reply(
				`Você não pode desativar o \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'command' : 'group'}.`
			);
		}
		args.cmdOrGrp.setEnabledIn(msg.guild, false);
		return msg.reply(`Desativou o \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'command' : 'group'}.`);
	}
};
