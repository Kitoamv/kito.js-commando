...
module.exports = class LoadCommandCommand extends Command {
	constructor(client) {
		super(client, {
			...
			details: oneLine`
			O argumento deve ser o nome completo do comando no formato \`group: memberName \`.
	...
	async run(msg, args) {
		...
		if(this.client.shard) {
			try {
				...
				await msg.reply(`Carregado \`${command.name}\` comando, mas falhou ao carregar em outros shards.`);
				return null;
			}
		}

		await msg.reply(`Carregado \`${command.name}\` Comando ${this.client.shard ? ' em todos os fragmentos' : ''}.`);
		return null;
	}
};
