	...
	async run(msg, args) { // eslint-disable-line complexity
		...
		if(args.command && !showAll) {
			if(commands.length === 1) {
				...
				`;
				if(commands[0].aliases.length > 0) help += `\n**Apelido:** ${commands[0].aliases.join(', ')}`;
				help += `\n${oneLine`
				...
				try {
					...
					if(msg.channel.type !== 'dm') messages.push(await msg.reply('Enviou um DM com informações.'));
				} catch(err) {
			...
			try {
				messages.push(await msg.direct(stripIndents`
					...
					`}
					Para executar um comando neste DM, basta usar ${Command.usage('comando', null, null)} sem prefixo.

					Use ${this.usage('<comando>', null, null)} para visualizar informações detalhadas sobre um comando específico.
					Use ${this.usage('all', null, null)} para visualizar uma lista de comandos * all *, não apenas os disponíveis.

					__**${showAll ? 'Todos os comandos' : `Comandos disponíveis em ${msg.guild || 'este DM'}`}**__
