...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`Você não tem permissão para usar o \`${this.command.name}\` comando.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`Eu preciso do "${permissions[missing[0]]}" permissão para o \`${this.command.name}\` comando para trabalhar.`
					);
				}
				return this.reply(oneLine`
					Eu preciso das seguintes permissões para o \`${this.command.name}\` comando para trabalhar:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`Você não pode usar o \`${this.command.name}\` comando novamente para outro ${remaining.toFixed(1)} segundos.`
			);
		}
		...
		try {
			this.client.emit('debug', `Comando em execução ${this.command.groupID}:${this.command.memberName}.`);
	...
	respond({ type = 'reply', content, options, lang, fromEdit = false }) {
		...
		switch(type) {
			...
			case 'code':
				...
				return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
			default:
				throw new RangeError(`Tipo de resposta desconhecido "${type}".`);
		}
	}
