...
class ArgumentCollector {
	...
	constructor(client, args, promptLimit = Infinity) {
		if(!client) throw new TypeError('O cliente do coletor deve ser especificado.');
		if(!args || !Array.isArray(args)) throw new TypeError('Args de coletor devem ser uma matriz.');
		...
		for(let i = 0; i < args.length; i++) {
			if(hasInfinite) throw new Error('Nenhum outro argumento pode vir após um argumento infinito.');
			if(args[i].default !== null) hasOptional = true;
			else if(hasOptional) throw new Error('Os argumentos necessários podem não vir após argumentos opcionais.');
